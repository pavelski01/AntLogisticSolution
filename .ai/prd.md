...
## 1. Product Overview
AntLogistics WMS MVP is a web application that lets warehouse teams record foundational data about warehouses, items, and stock levels. Built on .NET 10 with .NET Aspire 13.0 (AppHost, Core API, Astro+React UI) and PostgreSQL via Entity Framework Core, the codebase as of December 2025 delivers the foundational warehouse slice: Aspire orchestrates the Core API, applies migrations, and serves an Astro + React list view backed by warehouse APIs. Authentication, item catalog, and stock recording remain planned for upcoming iterations.

## 2. User Problem
Warehouse teams currently track stock movements manually (spreadsheets, email), which leads to errors, stale data, and poor visibility. Operators lack a single source of truth about locations, cannot quickly register stock levels, and logistics decisions rely on incomplete information. Basic access control to warehouse data is also missing. The MVP addresses these issues by providing a simplified WMS that supports adding warehouses and items, recording stock levels, and browsing up-to-date inventory once logged in.
...
- Stock entry for a selected warehouse with item, quantity, operator, and timestamp.
- UI data validation (required fields, numeric ranges, code uniqueness) with error messages in Polish across API and UI layers.
- List views for items and stock history with filters (name/code, date range).
- Inventory status calculation based on aggregated stock entries with automatic stock updates.
...
- User-facing documentation describing the flow: login → add warehouse → add item → record stock → view inventory.
...
- PostgreSQL data layer with Warehouse and Item models plus stock storage — entities and migrations in place, inventory calculations pending.
- Single user role, local login (bcrypt), no self-service registration — security slice not yet implemented.
- Basic stock handling without automatic allocation to bin locations — pending service and UI work.
...
Scenario: US-005 Record stock
   Given the operator is on the stock entry form with warehouse, item, quantity, and date/time
   And quantity must be greater than zero
   When the operator submits the stock entry
   Then the system stores the operator identity and UTC timestamp
   And the system immediately recalculates the inventory status by recording the stock quantity
...
Scenario: US-006 View warehouse inventory
   ...
   Then the view displays the calculated inventory status derived from current stock levels
   And the view shows columns warehouse, item, total quantity, last update without reloading the page
   And data refreshes after each stock entry to reflect the new inventory status
   And empty results display "No stock records found for selected filters."

Scenario: US-007 Handle invalid stock entries
   Given the stock entry form enforces validations on quantity and active selections
...
Scenario: US-008 Operator activity list
   ...
   When the system fetches the last n stock entries for the logged-in user
   Then the view shows date, warehouse, item, and quantity for each stock entry
   And the list updates automatically after new stock entries without a full reload
   And filters for warehouse and date limit the results
   And the UI indicates that CSV export is deferred beyond MVP

## 6. Success Metrics
1. 95% of stock entries recorded in the system within 2 minutes of physical inventory check (measured during pilot rollout).
...
4. Average API response time under 800 ms for stock entry creation under MVP load.
...
- Stories cover all MVP interactions (login, warehouse/item CRUD, stock entries, views).  
...
- Document stored at docs/prd.md for the team's follow-up work.
